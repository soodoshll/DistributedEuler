...
class TFTrainer(Trainer):
  ...
  def _train_epoch(self, train_op, loss, iterator, idx, timeline_name="timeline"):
      ...
      while True:
        try:
          ...
          if iter == 0:
            ...
            with open(timeline_name + ".json", 'w') as f:
              f.write(ctf)
          else:
            outs = self.sess.run([train_op, loss],
                               feed_dict=feed_dict)
...
class DistTFTrainer(TFTrainer):
  ...
  def init(self, **kwargs):
    self.sess = tf.train.MonitoredTrainingSession(
        ...
        is_chief=(self.task_index == 0), hooks = self.hooks)
  ...
  def train_and_evaluate(self, **kwargs):
    ...
    print('training...')
    for i in range(self._epoch):
      self._train_epoch(train_op, loss, train_iterator, i, str(self.task_index) + "_" + str(i))
